...
<link href="assets/datatables/jquery.dataTables.min.css" rel="stylesheet" type="text/css" />  
                    ...
                    <div class="col-lg-12	">
                        ...
                        <div class="panel panel-default"><!-- /primary heading -->
                            ...
                            <div class="panel-heading">
                                <h3 class="panel-title">Delivery Orders</h3>
                            </div>
 ...
 position:relative;" class="row">
                                    <table id="datatable1" class="table table-striped table-bordered">
                                            ...
                                            <tbody>
                                            <?php
$productsql = "SELECT * FROM `sw_deliveryorders` p
left join sw_proformas z on p.do_rel_po_id = z.po_id
left join sw_clients c on p.do_rel_cli_id = c.cli_id
left join sw_currency y on p.do_rel_cur_id = y.cur_id
WHERE p.do_revision = 0 and p.do_valid =1 and c.cli_valid =1
...
if ($productres->num_rows > 0) {
	...
	$getallrevisions = getdatafromsql($conn,"SELECT count(do_id) as revs FROM `sw_deliveryorders` WHERE do_revision_id =".$productrw['do_id']." and do_revision > 0");

		echo '<tr>
	<td>'.$con.'</td>
	<td>'.$productrw['do_ref'].'</td>
	<td>'.$productrw['po_ref'].'</td>
	<td>'.$productrw['do_proj_name'].'</td>
	<td>'.$productrw['do_subj'].'</td>
	...
	<td>1 '.$productrw['cur_name'].' = '.$productrw['do_cur_rate'].' AED </td>
	<td>'.(is_array($getallrevisions) ? $getallrevisions['revs'] : '0').'</td>
	<td>';
	$getrevisions = "SELECT * FROM sw_deliveryorders where do_revision_id = ".$productrw['do_id']." and do_valid =1";
...
if($getrevision['do_revision'] == 0){	echo'
<button id="getDetailedView" data-toggle="modal" data-target="#view-modal" data-id="'.md5($getrevision['do_id']).'" class="btn btn-sm btn-info">Main</button>
';}else{
	echo'
<hr>
<button id="getDetailedView" data-toggle="modal" data-target="#view-modal" data-id="'.md5($getrevision['do_id']).'" class="btn btn-sm btn-info">R'.$getrevision['do_revision'].' </button>
';
}
...
if($getrevision['do_revision'] == 0){	echo'
<button id="getPrintView" data-toggle="modal" data-target="#view-modal" data-id="'.md5($getrevision['do_id']).'" class="btn btn-sm btn-info">Main</button>
';}else{
	echo'<hr>

<button id="getPrintView" data-toggle="modal" data-target="#view-modal" data-id="'.md5($getrevision['do_id']).'" class="btn btn-sm btn-info">R'.$getrevision['do_revision'].' </button>
';
}
	...
	echo '</td>	
	<td>'.date('d-M-Y',$productrw['do_dnt']).'</td>
	<td>
<button id="getPoEdit" data-toggle="modal" data-target="#view-modal" data-id="'.md5($productrw['do_id']).'" class="btn btn-sm btn-warning">Revise</button>
	</td>
	</tr>';
...
<div class="row">
    <div class="col-xs-3">
        <div class="row"><h3 >&nbsp; Generate Delivery Order</h3></div>
        <div class="row">
            <h4 align="center">From Proforma</h4>
            <div class="row">
  ...
  <div class="modal-full modal-dialog"> 
     <div class="modal-content">  
        ...
        <div class="modal-header"> 
           ...
           <h4 class="modal-title">deliveryorder</h4> 
        </div> 
...
$(document).on('click', '#getModalForQuoteProAdd', function(e){  
     ...
     $.ajax({
          ...
          type: 'POST',
          data: 'add_deliveryorder_proforma='+uid,
          dataType: 'html'
     })
     .done(function(data){
          console.log(data); 
...
$(document).on('click', '#getPrintView', function(e){  
     ...
     $.ajax({
          url: 'page_that_gives_modal_popups_to_pages.php',
          type: 'POST',
          data: 'deliveryorders_print_view='+uid,
          dataType: 'html'
     })
     .done(function(data){
          console.log(data); 
...
$(document).on('click', '#getDetailedView', function(e){  
     ...
     $.ajax({
          url: 'page_that_gives_modal_popups_to_pages.php',
          type: 'POST',
          data: 'deliveryorder_detailed_view='+uid,
          dataType: 'html'
     })
     .done(function(data){
          console.log(data); 
